...
import { getDietPlans } from 'apiSdk/diet-plans';
...
import { DietPlanListPage as ShowCaseList } from 'pages/diet-plans';
...
function DashboardPage() {
  ...
  const dietPlanStat = useSWR<PaginatedInterface<DietPlanInterface>>(
    () => `/diet-plans`,
    () => getDietPlans({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Diet Plans',
      result: dietPlanStat?.data?.totalCount,
      isLoading: dietPlanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Dieticians',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`diet-plans`} passHref legacyBehavior>
                  <Button
