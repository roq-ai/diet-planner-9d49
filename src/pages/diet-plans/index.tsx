...
type ColumnType = ColumnDef<DietPlanInterface, unknown>;

interface DietPlanListPageProps {
...
export function DietPlanListPage(props: DietPlanListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getDietPlans({
        ...
        order: params.order,
        searchTermKeys: ['gender.contains', 'goal.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DietPlanInterface>>(
    () => `/diet-plans?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/diet-plans/view/${record.id}`} passHref legacyBehavior>
                <Button
